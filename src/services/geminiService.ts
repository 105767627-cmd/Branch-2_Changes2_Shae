...
const documentSchema = {
  ...
  properties: {
    ...
    grossAmount: { type: Type.NUMBER, description: "For Payroll: Total pay before tax (Gross) for this period." },
    taxWithheld: { type: Type.NUMBER, description: "For Payroll/Income: Total tax withheld (PAYG) for this period." },
    superannuation: { type: Type.NUMBER, description: "For Payroll: Super contribution amount for this period." },
    ytdGrossEarnings: { type: Type.NUMBER, description: "For Payroll: Year To Date (YTD) Gross Earnings." },
    ytdTaxWithheld: { type: Type.NUMBER, description: "For Payroll: Year To Date (YTD) Tax Withheld." },
    ...
    abn: { type: Type.STRING, description: "The Australian Business Number (ABN) of the vendor." },
    isCustomerCopy: { type: Type.BOOLEAN, description: "Set to true if the document explicitly states 'Customer Copy' or is only a card terminal receipt (EFTPOS receipt) without itemised details." },
      ...
      contents: [
        {
          parts: [
            ...
            4. For PAYROLL: Use 'Wages' or 'Salary'. Extract Gross Pay, Tax Withheld, and YTD (Year To Date) Gross Earnings and YTD Tax Withheld.
            ...
            8. AUDIT READINESS (CRITICAL):
               - Look for an 'ABN' (Australian Business Number). If missing, we need to know.
               - Look for an 'Invoice Number' or 'Receipt Number'.
               - Check if the document says 'Customer Copy' or is just a small EFTPOS terminal receipt (no line items).
            9. If image is blurry/ambiguous, set confidence='low' and explain why.` },
